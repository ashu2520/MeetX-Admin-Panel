...
const getPosts = async (req, res) => {
    ...
    try {
        const postsQuery = `
            SELECT p.*, a.post_type
            FROM posts p
            JOIN all_posts a ON p.all_post_id = a.id
            WHERE p.user_id = ? AND a.post_type = 'post'
        `;

        const [posts] = await db.execute(postsQuery, [userId]);
...
const getVideos = async (req, res) => {
    ...
    try {
        const videosQuery = `
            ...
            FROM posts p
            JOIN all_posts a ON p.all_post_id = a.id
            WHERE p.user_id = ? AND a.post_type = 'video'
        `;

        const [videos] = await db.execute(videosQuery, [userId]);
...
const getProfileMedia = async (req, res) => {
    ...
    const query = `
        SELECT profile_pic, profile_video
        FROM profiles
        WHERE user_id = ?
    `;
    ...
    try {
        ...
        res.status(200).json({
            ...
            profile_video: results[0].profile_video,
        });
    } catch (error) {
        console.error('Error fetching profile media:', error.message, error.stack);
        res.status(500).json({ error: 'Internal Server Error' });
    }
};
